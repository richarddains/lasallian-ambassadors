  ...
  const handleRegister = async () => {
    ...
    try {
      const res = await fetch('/api/registrations', {
        ...
        body: JSON.stringify({ eventId: event.id, volunteerType }),
      })
    ...
    <div className="-mx-8 -mt-8">
      ...
      <div className="max-w-7xl mx-auto px-10 py-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          {/* Slot availability */}
          {(event.lambSlots != null || event.docuLambSlots != null) && (
            <section>
              <h2 className="font-headline font-extrabold text-2xl text-emerald-900 mb-5">
                Slot Availability
              </h2>
              <div className="space-y-3">
                {event.lambSlots != null && (
                  <button
                    onClick={() => { if (event.status === 'PUBLISHED' && !isCoordinator && !registered) setVolunteerType('LAMB') }}
                    className={`w-full flex items-center justify-between p-5 rounded-xl transition-all group ${
                      volunteerType === 'LAMB' && !isCoordinator && !registered && event.status === 'PUBLISHED'
                        ? 'bg-primary text-white shadow-md'
                        : 'bg-white border border-slate-100 text-emerald-900 hover:bg-slate-50'
                    }`}
                  >
                    <div className="flex items-center gap-5">
                      <div className={`text-center px-4 py-2 rounded-lg min-w-[64px] ${volunteerType === 'LAMB' && !isCoordinator && !registered && event.status === 'PUBLISHED' ? 'bg-white/15' : 'bg-emerald-50'}`}>
                        <p className={`text-3xl font-headline font-black leading-none ${volunteerType === 'LAMB' && !isCoordinator && !registered && event.status === 'PUBLISHED' ? 'text-white' : 'text-emerald-800'}`}>
                          {String(event.lambSlots).padStart(2, '0')}
                        </p>
                        <p className={`text-[9px] font-bold uppercase tracking-tighter font-label mt-0.5 ${volunteerType === 'LAMB' && !isCoordinator && !registered && event.status === 'PUBLISHED' ? 'text-emerald-100' : 'text-slate-400'}`}>Slots</p>
                      </div>
                ...
                {event.docuLambSlots != null && (
                  <button
                    onClick={() => { if (event.status === 'PUBLISHED' && !isCoordinator && !registered) setVolunteerType('DOCULAMB') }}
                    className={`w-full flex items-center justify-between p-5 rounded-xl transition-all group ${
                      volunteerType === 'DOCULAMB' && !isCoordinator && !registered && event.status === 'PUBLISHED'
                        ? 'bg-primary text-white shadow-md'
                        : 'bg-white border border-slate-100 text-emerald-900 hover:bg-slate-50'
                    }`}
                  >
                    <div className="flex items-center gap-5">
                      <div className={`text-center px-4 py-2 rounded-lg min-w-[64px] ${volunteerType === 'DOCULAMB' && !isCoordinator && !registered && event.status === 'PUBLISHED' ? 'bg-white/15' : 'bg-emerald-50'}`}>
                        <p className={`text-3xl font-headline font-black leading-none ${volunteerType === 'DOCULAMB' && !isCoordinator && !registered && event.status === 'PUBLISHED' ? 'text-white' : 'text-emerald-800'}`}>
                          {String(event.docuLambSlots).padStart(2, '0')}
                        </p>
                        <p className={`text-[9px] font-bold uppercase tracking-tighter font-label mt-0.5 ${volunteerType === 'DOCULAMB' && !isCoordinator && !registered && event.status === 'PUBLISHED' ? 'text-emerald-100' : 'text-slate-400'}`}>Slots</p>
                      </div>
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-24 space-y-4">
            ...
            {/* Registration card */}
            {event.status === 'PUBLISHED' && !isCoordinator && (
              <div className="bg-emerald-950 text-white p-7 rounded-2xl shadow-xl shadow-emerald-900/10">
                <h3 className="font-headline font-bold text-xl mb-6">Registration</h3>
                ...
                {!registered ? (
                  ...
                  <div className="flex items-center gap-3 bg-emerald-800/50 border border-emerald-700/30 px-4 py-3 rounded-xl">
                    ...
                    <p className="font-label font-bold text-sm text-emerald-100">
                      Registered as {volunteerType === 'LAMB' ? 'LAmb' : 'DocuLAmb'}!
                    </p>
                  </div>
                )}
              </div>
